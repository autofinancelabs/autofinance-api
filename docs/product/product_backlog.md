...
- **Formato de historia:** `Como <rol>, quiero <acción>, para <beneficio>`. El rol operativo es
  el **asesor de crédito** (no se usa "usuario"; reservado a IAM).
...
## E1 — Autenticación y acceso `generic`

**H1.1 (Must)** — Como asesor de crédito, quiero iniciar sesión con usuario y contraseña, para
acceder al sistema.
```text
...
**H1.3 (Should)** — Como entidad, quiero registrar actores (asesores) habilitados, para controlar
quién puede operar.
...
**H2.1 (Must)** — Como asesor de crédito, quiero dar de alta los datos del cliente (deudor), para
...
**H2.2 (Must)** — Como asesor de crédito, quiero **editar y volver a guardar** los datos del
...
**H2.3 (Must)** — Como asesor de crédito, quiero consultar/listar clientes, para encontrar a quién
asociar una operación.

**H2.4 (Should)** — Como asesor de crédito, quiero dar de baja un cliente, para completar el CRUD.

---

## E3 — Gestión de ofertas vehiculares `supporting`

**H3.1 (Must)** — Como asesor de crédito, quiero registrar la oferta vehicular (vehículo, precio
...
**H3.2 (Must)** — Como asesor de crédito, quiero **editar y volver a guardar** la oferta vehicular,
para mantenerla actualizada.

**H3.3 (Must)** — Como asesor de crédito, quiero consultar/listar ofertas vehiculares.

**H3.4 (Should)** — Como asesor de crédito, quiero dar de baja una oferta, para completar el CRUD.

---

## E4 — Configuración del financiamiento `core (config)`

**H4.1 (Must)** — Como asesor de crédito, quiero definir la **moneda** de la operación (PEN o USD),
...
**H4.2 (Must)** — Como asesor de crédito, quiero definir el **tipo de tasa**: efectiva, o nominal
...
**H4.3 (Must)** — Como asesor de crédito, quiero definir al inicio los **periodos de gracia** total
...
**H4.4 (Must)** — Como asesor de crédito, quiero definir **% de cuota inicial**, **% de cuota
final (cuotón)** y el **plazo/Plan** (p. ej. Plan 36), para parametrizar la Compra Inteligente.

**H4.5 (Should)** — Como asesor de crédito, quiero configurar los **costos** de la operación
...
**H5.1 (Must)** — Como asesor de crédito, quiero generar el **cronograma por método francés vencido
...
**H5.2 (Must)** — Como asesor de crédito, quiero aplicar la **Compra Inteligente con cuotón**, para
...
**H5.3 (Must)** — Como asesor de crédito, quiero que la **gracia** se aplique correctamente, para
...
**H5.4 (Must)** — Como asesor de crédito, quiero que cada periodo arme el **flujo total**, para
...
**H5.6 (Should)** — Como asesor de crédito, quiero la variante con **desgravamen incorporado en la
...
**H6.1 (Must)** — Como asesor de crédito, quiero calcular el **VAN desde la óptica del deudor**,
...
**H6.2 (Must)** — Como asesor de crédito, quiero calcular la **TIR periódica**, para conocer el
...
**H6.3 (Must)** — Como asesor de crédito, quiero obtener la **TCEA**, para comparar el costo anual
real del crédito.
```text
...
**H6.4 (Must)** — Como asesor de crédito, quiero exponer los **indicadores de transparencia SBS**,
...
**H6.5 (Should)** — Como asesor de crédito, quiero ver la **conversión de tasas** aplicada
...
**H7.1 (Must)** — Como asesor de crédito, quiero **guardar** una simulación completa (configuración
...
**H7.2 (Must)** — Como asesor de crédito, quiero **reabrir y editar** una simulación guardada y
volver a calcular y guardar, para ajustar la operación (requisito de editar/reguardar).

**H7.3 (Should)** — Como asesor de crédito, quiero el **historial** de simulaciones por cliente,
...
**H8.1 (Should)** — Como asesor de crédito, quiero **ayuda contextual por campo** (qué es TEA,
